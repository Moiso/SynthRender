...
class KeyframesRenderer:
    ...
    def set_up_renderer(self, desired_gpu=None, verbose=False):
        output_dir = self.config["output_dir"] or "./output" # Set default output path if not specified.
        ...
        self.output_dir_hdf5 = os.path.join(self.output_dir, "hdf5")
        ...
        if "depth" in self.config["render_options"]:
            ...
            if self.config["save_raw"]:
                output_dir_depth = os.path.join(self.output_dir, "raw", "depth")
        ...
        if "normals" in self.config["render_options"]:
            ...
            if self.config["save_raw"]:
                output_dir_normals = os.path.join(self.output_dir,"raw", "normals")
        ...
        if "segmasks" in self.config["render_options"]:
            ...
            if self.config["save_raw"]:
                output_dir_segmasks = os.path.join(self.output_dir,"raw", "segmasks")
        ...
        if self.config["save_raw"]:
            self.output_dir_rgb = os.path.join(self.output_dir,"raw", "rgb")
    ...
    def render_scene(self, num_frames:int, seed=0, start_frame=0, stop_frame=-1, desired_gpu=None, verbose=False):
    ...
    def post_render(self):
        ...
        copy_path = os.path.join(self.output_dir, "config_copy.yaml")
