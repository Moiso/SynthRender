...
class ExternalCameraSetup:
    ...
    def create_frustum(self):
        ...
        material.nodes["Principled BSDF"].inputs[4].default_value = 0.1 # Alpha value

        material.nodes.remove(material.nodes["Principled BSDF"])
        
        nodes = material.nodes
        links = material.links

        # create a Transparent BSDF
        transp = nodes.new(type="ShaderNodeBsdfTransparent")
        transp.location = (-200, 0)

        # create an Emission shader (white, strength=1)
        em = nodes.new(type="ShaderNodeEmission")
        em.inputs["Color"].default_value = (1.0, 1.0, 1.0, 1.0)
        em.inputs["Strength"].default_value = 1.0
        em.location = (-200, -200)

        # create a Mix Shader and drive its Fac with your alpha
        mix_op = nodes.new(type="ShaderNodeMixShader")
        mix_op.inputs["Fac"].default_value = 0.05
        mix_op.location = (0, -100)

        # get the existing Material Output
        out = nodes.get("Material Output")
        out.location = (200, -100)

        # link Transparent → Mix Shader Shader1
        links.new(transp.outputs["BSDF"], mix_op.inputs[1])
        # link Emission → Mix Shader Shader2
        links.new(em.outputs["Emission"], mix_op.inputs[2])
        # link Mix Shader → Material Output
        links.new(mix_op.outputs["Shader"], out.inputs["Surface"])
