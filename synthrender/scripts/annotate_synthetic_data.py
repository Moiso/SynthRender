...
def parse_args() -> tuple[int, int, str]:
    ...
    parser.add_argument('--annotation', '-a', default="object_detection",  choices=["object_detection", "instance_segmentation"], type=str, help="Type of annotation to use.")

    args = parser.parse_args()
    
    return args.config, args.do_coco, args.do_bop, args.to_yolo, args.start_index, args.stop_index, args.output_dir, args.annotation
...
def generate_annotations(config:dict, hdf5_paths:list[str], do_coco:bool, do_bop:bool, keyframes:int=None, batch_size:int=200, output_dir=None):
    output_dir = output_dir or config["output_dir"] or "./output"
...
if __name__ == "__main__":
    config_path, do_coco, do_bop, to_yolo, start, stop, output_dir, annotation_type = parse_args()
    ...
    if do_coco or do_bop:
        hdf5_path  = os.path.join(cfg_output_dir,"hdf5")
    ...
    if to_yolo:
        coco_dir = os.path.join(cfg_output_dir, 'coco')
        category_mapping = config.get("coco_to_yolo_category_mapping")

        coco2yolo(coco_dir, category_mapping=category_mapping, verbose=True, annotation_type=annotation_type)