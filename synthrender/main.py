...
def parse_args() -> str:
    ...
    return args.config, args.num_frames, args.num_gpus, args.do_coco, args.do_bop, args.to_yolo

def run_annotator(script_path=None, config_path:str=None, do_coco:bool=True, do_bop:bool=False, to_yolo:bool=False, **kwargs):
    ret = False

    script_path = script_path or os.path.join(scripts_path[0], 'annotate_synthetic_data.py')
    ...
    try:
        ...
        if kwargs.get('annotation'): command.extend(['-a', kwargs.get('annotation')])
...
def create_dataset(config_file_path, n_frames, n_gpus, do_coco, do_bop, do_yolo):
    ...
    try:
        ...
        start = time.time()
        ret = run_parallel_create_syn_data("run", blenderproc_script, config_file_path, keyframes=n_frames, n_gpus=n_gpus)
...
if __name__ == "__main__":
    config_file_path, n_frames, n_gpus, do_coco, do_bop, do_yolo = parse_args()

    assert os.path.exists(config_file_path), f"Error: Could not find config file in: {config_file_path}"

    create_dataset(config_file_path, n_frames, n_gpus, do_coco, do_bop, do_yolo)